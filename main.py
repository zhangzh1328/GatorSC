...
def train(train_loader,
          ...
          save_model_name,
          device):
    model = Model(input_dim, num_head, hidden_dim, mlp_dim, num_hop, phi_1, phi_2, phi_3, 
                 del_rate, add_rate, max_path_len, tau, a1, a2, b1, b2, b3, lambda_1, lambda_2, dropout).to(device)
    ...
    for each_epoch in range(epochs):
        ...
        if cur_loss < min_loss:
            ...
            torch.save(state, save_model_name+'_'+str(int(seed))+'_dict')
...
def train_imputation(train_loader,
    ...
    model = Model(input_dim, num_head, hidden_dim, mlp_dim, num_hop, phi_1, phi_2, phi_3, 
                 del_rate, add_rate, max_path_len, tau, a1, a2, b1, b2, b3, lambda_1, lambda_2, dropout).to(device)
    ckpt_path = save_model_name+'_'+str(int(seed))+'_dict'
    ...
    for each_epoch in range(epochs):
        ...
        if cur_loss < min_loss:
            ...
            torch.save(state, save_model_name+'_imputation_'+str(int(seed))+'_dict')
...
def train_annotation(train_loader,
    ...
    for each_epoch in range(epochs):
        ...
        if cur_loss < min_loss:
            ...
            torch.save(state, save_model_name+'_annotation_'+str(int(seed))+'_dict')
...
def test(test_loader,
    ...
    if task == 'clustering':
        ...
        results = {"acc": acc, "nmi": nmi, "ari": ari, "homo": homo}
        ...
        min_loss = train_imputation(train_loader, valid_loader, input_dim, num_head, hidden_dim, mlp_dim, num_hop, phi_1, phi_2, phi_3, 
                                    ...
                                    lr, seed, epochs, save_model_name, device)
        model_imputation = ImputationRegressor(mlp_dim, input_dim).to(device)
        ckpt_path = save_model_name+'_imputation_'+str(int(seed))+'_dict'
        state = torch.load(ckpt_path, map_location=device)
        ...
        min_loss = train_annotation(train_loader, valid_loader, input_dim, num_head, hidden_dim, mlp_dim, num_hop, phi_1, phi_2, phi_3, 
                                    del_rate, add_rate, max_path_len, tau, a1, a2, b1, b2, b3, lambda_1, lambda_2, dropout, 
                                    lr, seed, epochs, n_clusters, save_model_name, device)
        model_annotation = CellTypeClassifier(mlp_dim, n_clusters).to(device)
        ckpt_path = save_model_name+'_annotation_'+str(int(seed))+'_dict'
        state = torch.load(ckpt_path, map_location=device)
        ...
        results = evaluate_multiclass_average([y_test_true], [y_test_pred])
...
if __name__ == '__main__':  
    ...
    parser.add_argument("--a2", type=float, default=0.85)
    parser.add_argument("--b1", type=float, default=0.65)
    parser.add_argument("--b2", type=float, default=0.1)
    parser.add_argument("--b3", type=float, default=0.65)
    parser.add_argument("--lambda_1", type=float, default=0.88)
    ...
    train_loss = train(train_loader, test_loader, input_dim, num_head, hidden_dim, mlp_dim, num_hop, 
                       phi_1, phi_2, phi_3, del_rate, add_rate, max_path_len, tau, a1, a2, b1, b2, b3, 
                       lambda_1, lambda_2, dropout, lr, seed, epochs, n_clusters, save_model_name, device)
    results = test(test_loader, input_dim, num_head, hidden_dim, mlp_dim, num_hop, phi_1, phi_2, phi_3, 
                   del_rate, add_rate, max_path_len, tau, a1, a2, b1, b2, b3, lambda_1, lambda_2, 
                   dropout, lr, seed, epochs, n_clusters, save_model_name, task, device)
