...
class Model(nn.Module):
    def __init__(self, input_dim, num_head, hidden_dim, mlp_dim, num_hop, phi_1, phi_2, phi_3, del_rate, add_rate, 
                 max_path_len, tau, alpha_1, alpha_2, alpha_3, beta_1, beta_2, gamma_1, gamma_2, lambda_1, dropout):
        super().__init__()
    ...
    def forward(self, input):
        ...
        loss_rec_1 = self.alpha_1 * loss_raw + self.alpha_2 * loss_aug 
        
        loss_cl_1 = final_cl_loss(self.beta_1, self.beta_2, global_cell_emb, global_cell_path_emb, global_cell_adj, global_cell_path_adj, tau=self.tau)
        ...
        loss_rec_2 = self.alpha_1 * loss_raw + self.alpha_2 * loss_aug 
        
        loss_cl_2 = final_cl_loss(self.beta_1, self.beta_2, global_gene_emb.T, global_gene_path_emb.T, global_gene_adj, global_gene_path_adj, tau=self.tau)
        
        loss_1 = self.gamma_1 * (loss_rec_1 + self.alpha_3 * loss_mgm_1) + self.gamma_2 * loss_cl_1    
        loss_2 = self.gamma_1 * (loss_rec_2 + self.alpha_3 * loss_mgm_2) + self.gamma_2 * loss_cl_2 
        
        loss = self.lambda_1 * loss_1 + (1 - self.lambda_1) * loss_2
